        ...
        try:
            ...
            if today_df.empty:
                ...
                if selected:
                    ...
                    with st.expander("✏️ 選択中のタスクを編集 / 削除"):
                        new_content = st.text_input("📝 内容修正", value=row["content"], key=f"content_{index}")
                        ...
                        with btn_col1:
                            if st.button("修正", key=f"edit_{index}"):
                                try:
                                    new_start_dt = datetime.combine(today, new_start)
                                    new_end_dt = datetime.combine(today, new_end)
            ...
            st.warning(f"タスク一覧の取得に失敗しました: {e}")
